#!/bin/bash
# Noyoupick Supabase Health Monitor — pure code, no LLM
# Checks: auth config, edge functions, recent signups, database health
# Posts alerts to #noyoupick only if problems found
# Run via cron every 30 min or heartbeat

set -euo pipefail

SUPABASE_TOKEN="sbp_3757ffdfb9e4e15d77771f93af8fbb4a937fce31"
PROJECT_REF="ygjlszswowvuzhaiafwg"
SLACK_TOKEN="${SLACK_TOKEN:-}"
CHANNEL="C0AD5K17QP3"  # #noyoupick
API="https://api.supabase.com/v1/projects/${PROJECT_REF}"
ANON_KEY="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InlnamxzenN3b3d2dXpoYWlhZndnIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njk2MDk1NzQsImV4cCI6MjA4NTE4NTU3NH0.KNDJ-Z381KL0ONP_lt32tnED7oEua13D4yhFwovUy7E"
...
# ─── 3. Test edge functions (dynamic) ───
echo "$FUNCTIONS" | python3 -c "
import sys, json
for f in json.load(sys.stdin):
    print(f['slug'])
" 2>/dev/null | while read -r slug; do
  STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X OPTIONS     "https://${PROJECT_REF}.supabase.co/functions/v1/$slug" 2>&1)
  if [[ "$STATUS" != "200" && "$STATUS" != "204" ]]; then
    add_warning "Edge function '$slug' OPTIONS returned $STATUS"
  fi
done
...
if [[ -n "$ALERTS" || -n "$WARNINGS" ]]; then
  REPORT=":stethoscope: *Noyoupick Supabase Health Check*\n_$(date -u '+%Y-%m-%d %H:%M UTC')_\n"
...
" 2>/dev/null
  ...
  echo "ALERTS POSTED to #noyoupick"
  exit 1
else
